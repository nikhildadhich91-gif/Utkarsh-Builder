        ...
        <StaggerContainer staggerChildren={0.2} className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 max-w-4xl mx-auto justify-center">
          {founders.map((founder, idx) => (
            ...
            >
              ...
              <div className="relative aspect-[1.2/1] w-full overflow-hidden bg-gray-100">
                <motion.img
        ...
        {selectedIdx !== null && (
          ...
          >
            ...
            <motion.div
              ...
              data-lenis-prevent
              className="relative bg-white rounded-[24px] sm:rounded-[32px] overflow-hidden border border-black/5 shadow-2xl max-w-2xl w-full flex flex-col items-center p-6 md:p-10 cursor-default max-h-[92vh] sm:max-h-[88vh] overflow-y-auto scrollbar-thin scrollbar-thumb-black/20 hover:scrollbar-thumb-[#C92C15] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:bg-black/20 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-[#C92C15]"
              transition={{ type: 'spring', stiffness: 300, damping: 30 }}
            >
              {/* Close Button */}
              <button
                onClick={() => setSelectedIdx(null)}
                className="absolute top-4 right-4 p-2 rounded-full bg-black/5 hover:bg-black/10 text-gray-600 hover:text-gray-900 transition-colors z-20 cursor-pointer"
              ...
              {/* Centered Image */}
              <div className="relative w-32 h-32 sm:w-40 sm:h-40 md:w-44 md:h-44 rounded-full overflow-hidden border-4 border-[#C92C15]/10 shadow-lg bg-gray-100 shrink-0 z-10">
                <motion.img
              ...
              <motion.div
                ...
                className="text-center mt-5 flex flex-col items-center w-full"
              >
                ...
                <h3 className="text-xl sm:text-2xl md:text-3xl font-extrabold mt-2.5 text-[#1B1B1B] tracking-tight">
                  {founders[selectedIdx].name}
                </h3>

                <div className="flex items-center gap-2 mt-2.5 text-gray-700 bg-[#C92C15]/5 px-4 py-2 rounded-xl border border-[#C92C15]/10">
                ...
                <div className="w-full border-t border-black/5 my-4 sm:my-5" />

                {/* The Full Readable Text */}
                <p className="text-sm md:text-base text-[#6F6F6F] font-light leading-relaxed px-2 md:px-6 text-center select-text">
                  {founders[selectedIdx].desc}
                </p>

                <div className="mt-6 flex items-center justify-between w-full border-t border-black/5 pt-4 text-xs font-bold text-[#C92C15] uppercase tracking-wider">
