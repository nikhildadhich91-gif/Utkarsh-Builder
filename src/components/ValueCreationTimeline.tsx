...
const AccordionTimelineMobile = ({ items }: { items: TimelineItem[] }) => {
  ...
  return (
    <div className="space-y-3 text-left py-2">
      {items.map((item) => {
        ...
        return (
          <div key={item.id} className="border border-black/5 rounded-xl bg-white overflow-hidden shadow-sm transition-all duration-300">
            <button
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                  isOpen ? "bg-[#C92C15] text-white animate-pulse" : "bg-[#FAF7F5] text-[#1B1B1B]"
                }`}>
                  <Icon size={16} />
                </div>
                <div>
                  <span className="text-[9px] font-bold text-[#6F6F6F] uppercase tracking-wide block">
                    {item.category}
                  </span>
                  <h3 className="text-xs sm:text-sm font-semibold text-[#1B1B1B] leading-tight">
                    {item.title}
                  </h3>
                </div>
              </div>
              <div className="text-[10px] font-mono font-semibold text-[#C92C15]">
                {item.date}
              </div>
            </button>
            ...
            {isOpen && (
              <div className="px-4 pb-4 pt-1 border-t border-black/5 animate-in fade-in slide-in-from-top-1 duration-200">
                <p className="text-xs text-[#6F6F6F] font-light leading-relaxed mb-3 mt-2">
                  {item.content}
                </p>
                {/* Progress bar */}
                <div className="w-full h-1 bg-black/5 rounded-full overflow-hidden mb-1.5">
                  <div
...
export const ValueCreationTimeline: React.FC = () => {
  const timelineData: TimelineItem[] = [
    ...
    },
    {
      ...
      date: 'Phase 05',
      content: 'Thorough quality checks, beautiful styling finishes, and keys handover with full support.',
      category: 'Delivery',
      icon: Award,
      relatedIds: [4, 6],
      status: 'pending',
  ...
  return (
    <section id="timeline" className="py-12 md:py-32 bg-[#FAF7F5] relative overflow-hidden border-t border-black/5">
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16 w-full relative z-10">
        
        {/* DESKTOP ONLY LAYOUT */}
        <div className="hidden md:grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center min-h-[600px]">
        ...
        <div className="block md:hidden space-y-6">
          ...
          <div className="text-left space-y-2">
            <span className="text-[#C92C15] text-xs font-bold tracking-[0.25em] uppercase block">Our Process</span>
            <h2 className="text-2xl font-bold text-[#1B1B1B] tracking-tight leading-tight">
              From Vision to Reality
            </h2>
            <p className="text-[#6F6F6F] font-light text-xs leading-relaxed">
              Every project follows our proven six-phase lifecycle.
            </p>
          </div>

          <AccordionTimelineMobile items={timelineData} />
        </div>
      </div>
    </section>
  );
