...
const useResponsiveDimension = (responsive: boolean, config: Record<string, unknown>, key: 'height' | 'width') => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${keyCap}`])
        v = config[`mobile${keyCap}`] as string | undefined;
      else if (w <= 768 && config[`tablet${keyCap}`])
        v = config[`tablet${keyCap}`] as string | undefined;
      else if (w <= 1024 && config[`desktop${keyCap}`])
        v = config[`desktop${keyCap}`] as string | undefined;
      setValue(v);
    };
...
export const GradualBlur: React.FC<GradualBlurProps> = (props) => {
  ...
  const config = useMemo(() => {
    ...
    return mergeConfigs(DEFAULT_CONFIG, presetConfig, props) as Required<GradualBlurProps> & Record<string, unknown>;
  }, [props]);
...
interface GradualBlurComponent extends React.NamedExoticComponent<GradualBlurProps> {
